#!/usr/bin/env bash
# run-dev.sh — сборка и запуск ДЕВ-инстанса в FOREGROUND (с логами).
#
# Отдельный сокет/слот gsettings/хоткей — не толкается с установленным
# gnome-clipboard-history-native (см. раздел «Дев-инстанс» в CLAUDE.md).
# Хоткей дев-меню: Super+Ctrl+B. Логи демона льются прямо в терминал;
# Ctrl+C — останавливает демон и чистит сокет.
#
# Запуск:  ./run-dev.sh          (Ctrl+C — стоп)
# Свой сокет/хоткей:  GCHN_SOCK=… GCHN_HOTKEY='<Super><Control>b' ./run-dev.sh
set -euo pipefail
cd "$(dirname "$0")"
...
# 3) Погасить прежний дев-демон и подчистить сокет. pkill -f (не -x): имя длиннее
# 15 симв. — ядро режет comm, поэтому матчим по полной командной строке.
